...
#include "ci_krembot_proximity_sensor.h"
...
namespace argos {
   ...
   CCI_KrembotProximitySensor::CCI_KrembotProximitySensor() :
      m_tReadings(24) {
   ...
   const CCI_KrembotProximitySensor::TReadings& CCI_KrembotProximitySensor::GetReadings() const {
     return m_tReadings;
   }
...
#ifdef ARGOS_WITH_LUA
   void CCI_KrembotProximitySensor::CreateLuaState(lua_State* pt_lua_state) {
...
#ifdef ARGOS_WITH_LUA
   void CCI_KrembotProximitySensor::ReadingsToLuaState(lua_State* pt_lua_state) {
   ...
   std::ostream& operator<<(std::ostream& c_os,
                            const CCI_KrembotProximitySensor::SReading& s_reading) {
   ...
   std::ostream& operator<<(std::ostream& c_os,
                            const CCI_KrembotProximitySensor::TReadings& t_readings) {
      if(! t_readings.empty()) {
